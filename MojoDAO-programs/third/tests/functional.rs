...
async fn initialize_success() {
    ...
    let account_data = context
        ...
        .await
        .unwrap();
    assert_eq!(account_data.data.cost, data.cost);
    assert_eq!(account_data.data.tier, data.tier);
...
async fn write_success() {
    ...
    assert_eq!(account_data.data.cost, new_data.cost);
    assert_eq!(account_data.data.tier, new_data.tier);
    assert_eq!(account_data.authority, authority.pubkey());
    assert_eq!(account_data.version, RecordData::CURRENT_VERSION);
}
